...
images: [
  '/images/warehouse-1.jpg',
  {
    ...
    caption: 'Закрытая рампа',
    alt: 'Закрытая рампа склада Малдис в Могилёве',
  },
],
```

Подпись `caption` показывается снизу на фото. `alt` читают поисковики и скринридеры — лучше писать, что на снимке и где это. Если `alt` нет, подставляется `caption` или название склада. Если фото одно — карусель показывает только его, без стрелок.
